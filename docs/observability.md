...
指标：`kim_channel_total`、`kim_message_in_total`、`kim_message_in_flow_bytes`、`kim_message_out_flow_bytes`、`kim_no_server_found_error_total`（仅当 forward 错误正好是 `no adult instances`）、`kim_login_total`、`kim_handler_duration_seconds`（command 白名单，其它 → `other`）、`kim_talk_total`、`kim_session_not_found_total`。
