...
| 序 | 覆盖 | 依赖 | 规格 |
|---:|---|---|---|
| 6 | SIGTERM + 先摘发现再 drain（G-07、G-32） | 无 | 本阶段外 |
| 7 | 心跳 Redis 有界宽限（G-31） | 无 | 本阶段外 |
| 8 | `TcpConn<S>` + TGateway TLS（G-34） | 形状已在 G-34 拍板 | 本阶段外 |
| 9 | Mobile 成熟化：kim-client supervisor/sync、Dart 状态重排 + outbox、自研 ChatList、UI token v2 | 无服务端改动 | [06-mobile-client-maturity.md](./06-mobile-client-maturity.md) |
