...
`kim-client` 仍是协议层（session / login / talk / ack）。本地业务状态的唯一写入者是 workspace crate **`kim-sdk`**：store、outbox、未读、persist-then-ack。Flutter 是壳 + 平台适配。FFI 唯一 opaque 对象是 `KimSdkHandle`（`create` 始终拥有协议；`attach_store` 仅 runtime flag `kim.rustStore` 开时打开 `kim-cache.db`）。

`kim-client` 有 `inbox_list` / `history` / `offline_index` / `offline_content`、统一 `send_message(dest, kind, content, client_id)`，以及 `SessionSupervisor`（重连退避 + `SyncEngine`）。有 `PersistHook` 时 SyncEngine 在自己的 task 上 persist(Keep) 后 `ack_batch`（空页仍 ack）；Live Talk `try_send`，满则不 ACK、不停读循环。无 hook 时仍 ConfirmGate。Flutter FFI `KimSdkHandle`：`create/start_session/stop/link_state/session_events/watch_session/watch_timeline/sync_confirm/notify_radio_up/send_message/history/inbox`。`rustStore` 关时 Dart isolate 仍拥有 SQLite。聊天页是自研 reverse `ChatList`。Web `isRetryable` 对 `ServiceUnavailable=3` 与 3xx 重试。漏 Push 补偿仍是 G-03；G-13 客户端凭证持久化另项。
...
- 产品 UI：`go_router` 底部三栏（消息 / 通讯录 / 我）+ 会话页；`flutter_riverpod` **3.4**（`AuthNotifier` + `linkProvider` 镜像 supervisor、`threadsProvider` / `threadMessagesProvider.family` / `outboxProvider`、`Mutation` 管登录/发消息/好友副作用、`kimRetry` 只重试瞬时网络错误、`ref.mounted` 挡住作废的 async）。`linkProvider` 与 `outboxProvider` 必须在 `KimApp` 根上 watch，不能放进 IndexedStack tab（3.0 会 pause 离屏 listener）。聊天页是自研 `ChatList`（reverse `CustomScrollView`，`ChatListController.jumpToMessage` / `atBottomEdge` / `scrollToBottom`，键盘跟底 / 历史锚定，向上 `loadOlder`，新消息浮条，未读分界），数据来自 `threadMessagesProvider`；`flex_color_scheme` 作 M3 语义底，叠 `KimTheme` v2 token（Telegram 字阶/圆角/4pt 栅格/180-260ms easeOutCubic，浅色画布 #F2F5F8 / 深色 #0E1621）。`flutter_animate`、`wolt_modal_sheet`、`flutter_slidable`、`toastification`、`skeletonizer`、`lucide_icons_flutter` 负责动效和常见交互。主题是 **Material 3**（跟随系统亮暗）。聊天 / 改密走宽左缘跟手返回（约 80px，不靠 iOS 20px / Android 系统预测性返回）。不用 Cupertino 当 app theme，不用 Dart `web_socket_channel`。`main()` 先 `runApp` splash，bootstrap（路径/SQLite/FFI）完成后再进 `KimApp`。登录后 supervisor 自动连上。不再露出 ping / talk 调试按钮。
- 会话列表和消息缓存在 SQLite（按账号，增量 `upsert` + `loadMessagesPage`）。文本 / 图片 / 视频发送都走 Dart outbox（sending 落库 → 图片先 `uploadImage` 再 `sendMessage`，视频同样走 `sendMessage`；稳定 `client_id` → sent/failed，失败行可重试，上线重放）。列表缩略图走 `cached_network_image`（内存按显示宽解码 + 磁盘缓存，与预览共用 `CachedNetworkImageProvider`）。点击全屏：`photo_view`（pinch / 双击缩放、拖动）+ `dismissible_page`（纵向滑关闭、透明路由 Hero）。通讯录 / 好友申请 / 搜索走 `kim-client` 的 `chat.friend.*` 与 `chat.user.search`。私聊发送前要求已是好友（`NotFriends=109` 时输入栏换成加好友）。
