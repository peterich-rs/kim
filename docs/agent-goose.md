...
The client flag `agent.server_identity` defaults **on** for desktop (`agentHostSupported`). When off, 1:1 stays
...
`agent.multi_profile` is a kill-switch that also defaults on for desktop.

## Product

1. Open **我 → Agent**. List / edit personas at `/agent` and `/agent/:id`; provider keys at `/agent/accounts`. `/agent/settings` redirects to `/agent`.
2. Pick a catalog vendor (DeepSeek, Claude, …). Reasoning controls come from `catalog_surface` (DeepSeek `none|low|high|max`, Claude `low|high|max`). Fetch models is a button. Custom OpenAI-compatible needs https (loopback http allowed).
3. **助手** is a contact (通讯录) and a session in 消息. New / duplicate / wizard profiles register immediately when the identity flag is on; an existing empty `serverAccount` registers on first 1:1 open. Login / `ConnStatus.online` does **not** batch-ensure. Deleting a persona (not goose) calls `botDelete` then drops the local row.
4. Tap it and chat like a friend. Registered 1:1 messages are stored on the server so phone / web can read and send. Goose still runs only on this desktop; the owner session posts `chat.bot.reply`.
5. In a human chat you can still `@助手` / `@goose` to pull the same local agent in. Those replies stay on this device.

Shape: [impl/multi-agent-vendor-catalog.md](impl/multi-agent-vendor-catalog.md).
