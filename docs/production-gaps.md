...
切片 1（G-02 + G-08 的 **Chat 长连接** 边界）已落地。**不**把 G-02/G-08 标已关闭：Royal 裸 HTTP 仍在，依赖切片 3（G-01）。G-05 单租户 vs 贯穿 `app` 仍须拍板后才能写切片 4。切片 2 设计见 [impl/02-persist-first.md](impl/02-persist-first.md)；G-09 等代码合入后再从本文件删除。
