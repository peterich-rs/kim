...
自动部署会同步并校验 `consul/agent.hcl`、`consul/acl-init.sh` 为普通文件；旧版短挂载误建的同名空目录也会在上传前清理。Compose 的 Consul 文件挂载禁止自动建目录，缺文件会立即报错。Consul agent 以 uid 100 读取 server / CA 私钥和 `secrets.hcl`；业务容器通过专用补充组 gid 10001 只读取共享 client key。部署失败会自动打印 Consul、ACL 和业务服务日志；手工排障也可看 `docker logs kim-consul-1` / `docker logs kim-royal-1`：`permission denied` 是 TLS/secrets 文件权限不匹配（bootstrap 会修复）；`encryption key` / `invalid config` 是旧数据卷。
