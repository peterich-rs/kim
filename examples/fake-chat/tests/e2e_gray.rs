...
use fake_gateway::{GatewayHandler, KickHook, Route, RouteFile, RouteSelector, ZoneFile};
use harness::*;
...
async fn whitelist_app_hits_chat2_then_fallback() {
    ...
    let h1 = Arc::new(ChatHandler::with_seams_and_zone(
        c1.clone(),
        cache.clone(),
        Arc::new(fake_chat::store::MemoryMessageStore::new(Arc::new(
            fake_chat::idgen::SequenceIdGen::new(10_001),
        ))),
        Arc::new(fake_chat::directory::MemoryGroupDirectory::new(Arc::new(
            fake_chat::idgen::SequenceIdGen::new(20_001),
        ))),
        "zone_local".into(),
    ));
    ...
    let h2 = Arc::new(ChatHandler::with_seams_and_zone(
        ...
        cache.clone(),
        Arc::new(fake_chat::store::MemoryMessageStore::new(Arc::new(
            fake_chat::idgen::SequenceIdGen::new(11_001),
        ))),
        Arc::new(fake_chat::directory::MemoryGroupDirectory::new(Arc::new(
            fake_chat::idgen::SequenceIdGen::new(21_001),
        ))),
        "zone_gray".into(),
    ));
