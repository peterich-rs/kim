...
265 rules across 26 categories. Open a rule file when the task needs that detail. Design order lives in [SKILL.md](../SKILL.md). Security constraints (unwrap, unsafe, secrets, locks, release profile) live in [strict.md](strict.md). When a rule disagrees with those two, follow them.
...
- [`own-borrow-over-clone`](../rules/own-borrow-over-clone.md) - Borrow when the callee only reads; clone when a new owner needs the value
...
4. **Read rule files** under `rules/` for examples. Release-profile defaults are in [strict.md](strict.md)
