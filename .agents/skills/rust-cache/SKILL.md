...
name: rust-cache
description: >
  Redis caching in Rust: connection pools, TTL, Cache-Aside, Write-Through,
  invalidation, stampede protection, and SCAN-based deletes. Use when adding
  Redis, session/online-status caches, or cache invalidation. Invoke with
  /rust-cache.
license: MIT
metadata:
  author: huiali
...
- `rust-async-patterns` for Tokio Redis I/O
- `rust-database` when the cache sits in front of SQL
- `rust-distributed` for distributed locks beyond a single Redis key
